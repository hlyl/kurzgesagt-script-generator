...
class Shot(BaseModel):
    ...
    narration: str = Field(..., min_length=1, description="Voice-over narration for this shot")
    duration: int = Field(..., ge=1, le=60, description="Shot duration in seconds")
    description: str = Field(..., min_length=1, description="What this shot accomplishes")
    ...
    is_nested: bool = Field(default=False, description="Whether shot contains nested camera moves")
    transition_note: Optional[str] = Field(None, description="How to transition to next shot")
